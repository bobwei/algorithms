...
var maxStudents = function(seats) {
  ...
  const selections = new Array(m).fill(null).map((_, i) => createSelections(seats, m, n, i));
  let dp = {};
  for (const selection of selections[0]) {
    dp[selection.val] = selection.nSelected;
  }
  let max = 0;
  for (let i = 1; i < m; i++) {
    const next = {};
    for (const selection1 of selections[i]) {
      next[selection1.val] = 0;
      for (const selection2 of selections[i - 1]) {
