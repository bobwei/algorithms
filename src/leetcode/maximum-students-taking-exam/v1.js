...
var maxStudents = function(seats) {
  ...
  for (const selection of createSelections(seats, m, n, 0)) {
    dp[0][selection.val] = selection.nSelected;
  }
  let max = 0;
  for (let i = 1; i < m; i++) {
    for (const selection1 of createSelections(seats, m, n, i)) {
      dp[i][selection1.val] = 0;
      for (const selection2 of createSelections(seats, m, n, i - 1)) {
        if (isValid(n, selection1, selection2)) {
          dp[i][selection1.val] = Math.max(
            dp[i][selection1.val],
            selection1.nSelected + dp[i - 1][selection2.val],
          );
        }
      }
      max = Math.max(max, dp[i][selection1.val]);
    }
  }
  return max;
};
